...
# I messed up re.sub because it might have matched multiple times or incorrectly
# Let's use a more robust way to replace the whole block from startRecording to end of recordFrame

start_rec_pos = content.find('void startRecording() {')
usb_msc_pos = content.find('//  USB MSC')

if start_rec_pos != -1 and usb_msc_pos != -1:
    new_recording_block = """void startRecording() {
...
"""
    content = content[:start_rec_pos] + new_recording_block + content[usb_msc_pos:]
